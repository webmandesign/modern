			...
			// Processing
				...
				// Styles
					...
					// RTL setup
						...
						wp_style_add_data( 'modern-customize-controls', 'rtl', 'replace' );
		...
		public static function preview_scripts() {
				...
				if (
					...
					foreach ( $theme_options as $theme_option ) {
						if (
							...
							$output_single = (string) apply_filters( 'wmhook_modern_library_customize_preview_scripts_option_' . $option_id, $output_single );
			...
			// Output
				...
				if ( $output = trim( $output ) ) {
					echo (string) apply_filters( 'wmhook_modern_library_customize_preview_scripts_output', '<!-- Theme custom scripts -->' . PHP_EOL . '<script type="text/javascript"><!--' . PHP_EOL . '( function( $ ) {' . PHP_EOL.PHP_EOL . trim( $output ) . PHP_EOL.PHP_EOL . '} )( jQuery );' . PHP_EOL . '//--></script>' );
				}
		...
		public static function customize( $wp_customize ) {
			...
			// Helper variables
				...
				// To make sure our customizer sections start after WordPress default ones
					...
					$priority = absint( apply_filters( 'wmhook_modern_library_customize_priority', 0 ) );

				// Default section name in case not set (should be overwritten anyway)

					$customizer_panel   = '';
					$customizer_section = 'modern';
			...
			// Processing
				...
				// Custom controls
					...
					require_once MODERN_LIBRARY . 'includes/classes/class-customize-control-hidden.php';
					require_once MODERN_LIBRARY . 'includes/classes/class-customize-control-html.php';
					require_once MODERN_LIBRARY . 'includes/classes/class-customize-control-multiselect.php';
					require_once MODERN_LIBRARY . 'includes/classes/class-customize-control-radio-matrix.php';
					require_once MODERN_LIBRARY . 'includes/classes/class-customize-control-select.php';

					do_action( 'wmhook_modern_library_customize_load_controls', $wp_customize );
...
} // /Modern_Library_Customize

add_action( 'after_setup_theme', 'Modern_Library_Customize::init', 20 );
