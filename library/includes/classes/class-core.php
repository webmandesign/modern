	 ...
	 */
		...
		public static function theme_upgrade() {
			...
			// Helper variables
				...
				$current_theme_version = get_transient( 'modern_version' );
				$new_theme_version     = wp_get_theme( 'modern' )->get( 'Version' );
	 ...
	 */
		...
		public static function add_table_of_contents( $content = '' ) {
			...
			// Helper variables
				...
				$title      = apply_filters( 'wmhook_modern_library_add_table_of_contents_title', '<h2 class="screen-reader-text">' . $title_text . '</h2>' );
				...
				// Get post parts titles
					...
					foreach ( $args['post_content'] as $part ) {
						...
						// Get the title for post part
							...
							if ( $args['disable_first'] && 1 === $i ) {
								...
								if ( ! isset( $matches[2] ) || ! $matches[2] ) {
									$part_title = sprintf( esc_html__( 'Page %s', 'modern' ), number_format_i18n( $i ) );
								} else {
						...
						// Post part item output
							...
							$args['links'][$i] = (string) apply_filters( 'wmhook_modern_library_add_table_of_contents_part', '<li' . $class . '>' . _wp_link_page( $i ) . $part_title . '</a></li>', $i, $part_title, $class, $args );
		...
		public static function get_the_paginated_suffix( $tag = '', $singular_only = false ) {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_modern_library_get_the_paginated_suffix_pre', false, $tag, $singular_only );
			...
			// Processing
				...
				if ( 1 < $paginated ) {
					$output = ' ' . $tag[0] . sprintf( esc_html_x( '(page %s)', 'Paginated content title suffix, %s: page number.', 'modern' ), number_format_i18n( $paginated ) ) . $tag[1];
				}
