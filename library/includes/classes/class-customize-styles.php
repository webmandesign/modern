 ...
 * 100) Helpers
 */
final class Modern_Library_Customize_Styles {
	 ...
	 */
		...
		public static $cache_key = 'modern_custom_css';
				...
				// Hooks
					...
					// Actions
						...
						if ( self::$supports_generator ) {
							...
							add_action( 'wmhook_modern_library_theme_upgrade', __CLASS__ . '::generate_main_css_all' );
							...
							add_action( 'wmhook_modern_library_theme_upgrade', __CLASS__ . '::custom_styles_cache_flush' );

						}

						add_action( 'wmhook_modern_library_generate_main_css', __CLASS__ . '::stylesheet_timestamp' );
					...
					// Filters
						...
						// Minify CSS
							...
							add_filter( 'wmhook_modern_library_generate_main_css_output_min', __CLASS__ . '::minify_css' );
							add_filter( 'wmhook_modern_library_custom_styles_output_cache',   __CLASS__ . '::minify_css' );

						// SSL ready URLs

							add_filter( 'wmhook_modern_library_generate_main_css_output', 'Modern_Library::fix_ssl_urls', 9999 );
							add_filter( 'wmhook_modern_library_custom_styles_output',     'Modern_Library::fix_ssl_urls', 9999 );
	 ...
	 */
		...
		public static function generate_main_css( $args = array() ) {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_modern_library_generate_main_css_pre', ! self::$supports_generator, $args );
			...
			// Helper variables
				...
				$args = apply_filters( 'wmhook_modern_library_generate_main_css_args', wp_parse_args( $args, array(
						'type' => '',
					) ) );
			...
			// Processing
				...
				// Get the file content with output buffering
					...
					ob_start();
					require_once MODERN_PATH . 'assets/css-generate/generate-css' . $args['type'] . '.php';
					$output = apply_filters( 'wmhook_modern_library_generate_main_css_output', trim( ob_get_clean() ), $args );
				...
				// Minify output if set
					...
					$output_min = apply_filters( 'wmhook_modern_library_generate_main_css_output_min', $output, $args );

				// Create the theme CSS folder

					$wp_upload_dir = wp_upload_dir();

					$theme_css_url = trailingslashit( $wp_upload_dir['baseurl'] ) . 'wmtheme-modern';
				...
				// Create the theme CSS files
					...
					$file_name = apply_filters( 'wmhook_modern_library_generate_main_css_file_name', 'modern-styles' . $args['type'], $args );

					$global_css_path     = apply_filters( 'wmhook_modern_library_generate_main_css_global_css_path', trailingslashit( $theme_css_dir ) . $file_name . '.css', $args, $file_name, $theme_css_dir );
					$global_css_path_dev = apply_filters( 'wmhook_modern_library_generate_main_css_global_css_path_dev', trailingslashit( $theme_css_dir ) . 'dev-' . $file_name . '.css', $args, $file_name, $theme_css_dir );
					...
					if (
						...
						// Run custom actions
							...
							do_action( 'wmhook_modern_library_generate_main_css', $args );
			...
			public static function generate_main_css_rtl() {
				...
				// Output
					...
					if ( apply_filters( 'wmhook_modern_enable_rtl', false ) ) {
	 ...
	 */
		 ...
		 * or hooking it onto `wmhook_modern_custom_styles` filter.
		 *
		 * @uses  `wmhook_modern_theme_options` global hook
		 * @uses  `wmhook_modern_custom_styles` global hook
		 * @uses  `wmhook_modern_custom_styles_alphas` global hook
		...
		public static function custom_styles( $css = '', $scope = '' ) {
			...
			// Requirements check
				...
				$css = trim( (string) apply_filters( 'wmhook_modern_custom_styles', $css, $scope ) );
				...
				if (
					...
					if ( $output_cached = trim( (string) apply_filters( 'wmhook_modern_library_custom_styles_output', $output_cached, $scope ) ) ) {
			...
			// Helper variables
				...
				$rgba_alphas   = array_filter( (array) apply_filters( 'wmhook_modern_custom_styles_alphas', array() ) );
					...
					if ( ! empty( $theme_options ) ) {
						...
						foreach ( $theme_options as $option ) {
							...
							// Value filtering
								...
								$value = apply_filters( 'wmhook_modern_library_custom_styles_value', $value, $option );
						...
						$replacements = (array) apply_filters( 'wmhook_modern_library_custom_styles_replacements', $replacements, $theme_options, $css, $scope );
			...
			// Output
				...
				if ( $output = trim( (string) apply_filters( 'wmhook_modern_library_custom_styles_output', $output, $scope ) ) ) {
					return $output;
				}
	 ...
	 */
		...
		public static function get_filesystem() {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_modern_library_get_filesystem_pre', false );
	 ...
	 */
		...
		public static function color_hex_to_rgba( $hex, $alpha = 100 ) {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_modern_library_color_hex_to_rgba_pre', false, $hex, $alpha );
...
} // /Modern_Library_Customize_Styles

add_action( 'after_setup_theme', 'Modern_Library_Customize_Styles::init', 20 );
